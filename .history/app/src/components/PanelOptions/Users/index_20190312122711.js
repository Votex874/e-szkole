...
class Users extends Component{
  ...
  handleShowMoreInfo = () => {
    ...
    console.log(currentArrowImg)
  }

  render(){
    const { sidebarItems, users, currentArrowImg } = this.state
    return (
      <React.Fragment>
          ...
          {users.map((e, i) => {
            return (
              <UserItem key={i}>
                <Id>{i + 1}.</Id>  
                <UserName>{e}</UserName>
                <Icons><Img src={Trash} /> <Img onClick={this.handleShowMoreInfo} src={currentArrowImg} /></Icons>
              </UserItem>
            )
          })}
        </UsersList>
